...
object ExecutorsHeuristic {
  ...
  class Evaluator(executorsHeuristic: ExecutorsHeuristic, data: SparkApplicationData) {
    ...
    lazy val taskTimeDistribution: Distribution =
      Distribution(executorSummaries.map {executionSummary => executionSummary.endTime.getOrElse(appAttemptInfo.endTime).getTime - executionSummary.addTime.getTime })

    lazy val totalTaskTime : Long = executorSummaries.map(executionSummary => executionSummary.endTime.getOrElse(appAttemptInfo.endTime).getTime - executionSummary.addTime.getTime).sum
